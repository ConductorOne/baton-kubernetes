...
| Nodes | Opt-in (`--sync-nodes`) | |
| Pods | Opt-in (`--sync-pods`) | |
| Deployments | Opt-in (`--sync-deployments`) | |
| StatefulSets | Opt-in (`--sync-stateful-sets`) | |
| DaemonSets | Opt-in (`--sync-daemon-sets`) | |
| Secrets | Opt-in (`--sync-secrets`) | |
| ConfigMaps | Opt-in (`--sync-config-maps`) | |

<Note>
Workload and configuration resource types (Nodes, Pods, Deployments, StatefulSets, DaemonSets, Secrets, ConfigMaps) are excluded from the default sync and can be enabled individually with the CLI flags shown above.
</Note>
