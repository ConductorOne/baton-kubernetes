...
— on the test cluster that was 78% of all object-level access, none of it anything a reviewer
acts on. What those roles permit is still shown on the API resources. Turn on **Include
control-plane permissions on objects** (`--include-system-object-permissions`) if you want them
back on the objects too.

<Warning>
Object-level permissions scale with the number of objects, and pods are the type to watch: each
pod declares 25 permissions and carried 11 grants on the test cluster (91 with control-plane
permissions included). On a cluster with 5,000 pods that is around 125,000 rows for pods alone,
so consider what opting pods in means at your scale. Pods are marked as a resource type whose
count may legitimately fall between syncs, so a rollout replacing every pod is not reported as
missing data.
</Warning>
